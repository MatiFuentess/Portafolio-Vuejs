
<script setup>
import { ref } from "vue"

// Definir las habilidades en la constante 'habilidades'
const habilidades = ref([
    {
       id: 1, nombre: "Lenguajes de programacion", habilidades: [
            { id: 1, nombre: "Python", nivel: "Intermedio", icono: "/src/components/icons/python1.svg" },
            { id: 2, nombre: "JavaScript", nivel: "Intermedio", icono: "/src/components/icons/javascript.svg" },
            { id: 3, nombre: "Html", nivel: "Intermedio", icono: "/src/components/icons/html.svg" },
            { id: 4, nombre: "Css", nivel: "Intermedio", icono: "/src/components/icons/css.svg" },
            { id: 5, nombre: "Sql", nivel: "Intermedio", icono: "/src/components/icons/sql.svg" },
            { id: 6, nombre: "Java", nivel: "Intermedio", icono: "/src/components/icons/java.svg" },
        ]
    },
    {
        id: 2, nombre: "Framework y Librerias", habilidades: [
            { id: 1, nombre: "Vue.js", nivel: "Intermedio", icono: "/src/components/icons/vue.svg" },
        ]
    },
    {
        
        id: 3, nombre: "Herramientas y Software", habilidades: [
            { id: 1, nombre: "Git", nivel: "Intermedio", icono: "/src/components/icons/git.svg" },
            { id: 2, nombre: "Visual Studio Code", nivel: "Intermedio", icono: "/src/components/icons/visualstudiocode.svg" },
        ]
    },
    {
         id: 4, nombre: "Base de Datos", habilidades: [
            { id: 1, nombre: "MySQL", nivel: "Intermedio", icono: "/src/components/icons/mysql.svg" },
        ]
    },
    {
            id: 5, nombre: "Idiomas", habilidades: [
            { id: 1, nombre: "Español", nivel: "Nativo", icono: "/src/components/icons/arg.svg" },
            { id: 2, nombre: "Inglés", nivel: "Intermedio", icono: "/src/components/icons/eeuu.svg" },
        ]
    }
]);

</script>

<template>
    <div class="skills-contenedor">
        <!-- Iterar sobre las categorías de habilidades -->
        <div v-for="habilidad in habilidades" :key="habilidad.id" class="skills-categoria">
            <h3>{{ habilidad.nombre }}</h3>
            <ul class="skills">
                <!-- Iterar sobre los items dentro de cada categoría -->
                <li v-for="habilidad in habilidad.habilidades" :key="habilidad.id" class="skill">
                    <img :src="habilidad.icono" :alt="habilidad.nombre">
                    <span>{{ habilidad.nombre }}: {{ habilidad.nivel }}</span>
                </li>
            </ul>
        </div>
    </div>
</template>



<!-- <template>
    <div class="skills-contenedor">
        <div class="skills-categoria">
            <h3>Lenguajes de programacion</h3>
            <ul class="skills">
                <li class="skill">
                    <img src="/src/components/icons/python1.svg" alt="Python">
                    <span>Python: Intermedio</span>
                </li>
                <li class="skill">
                    <img src="/src/components/icons/javascript.svg" alt="JavaScript">
                    <span>JavaScript: Intermedio</span>
                </li>
                <li class="skill">
                    <img src="/src/components/icons/html.svg" alt="Html">
                    <span>Html: Intermedio</span>
                </li>
                <li class="skill">
                    <img src="/src/components/icons/css.svg" alt="Css">
                    <span>Css: Intermedio</span>
                </li>
                <li class="skill">
                    <img src="/src/components/icons/sql.svg" alt="Sql">
                    <span>Sql: Intermedio</span>
                </li>
                <li class="skill">
                    <img src="/src/components/icons/java.svg" alt="Java">
                    <span>Java: Intermedio</span>
                </li>
            </ul>
        </div>
    

    <div class="skills-categoria">
        <h3>Frameworks y Librerias</h3>
        <ul class="skills">
            <li class="skill">
                <img src="/src/components/icons/vue.svg" alt="Vue.js">
                <span>Vue.js: Intermedio</span>
            </li>
        </ul>
    </div>

    <div class="skills-categoria">
        <h3>Herramienta y Software</h3>
        <ul class="skills">
            <li class="skill">
                <img src="/src/components/icons/git.svg" alt="Git">
                <span>Git: Intermedio</span>
            </li>
            <li class="skill">
                <img src="/src/components/icons/visualstudiocode.svg" alt="Visual Studio Code">
                <span>Visual Studio Code: Intermedio</span>
            </li>
        </ul>
    </div>

    <div class="skills-categoria">
        <h3>Base de Datos</h3>
        <ul class="skills">
            <li class="skill">
                <img src="/src/components/icons/mysql.svg" alt="MySQL">
                <span>MySQL: Intermedio</span>
            </li>
        </ul>
    </div>

    <div class="skills-categoria">
        <h3>Idiomas</h3>
        <ul class="skills">
            <li class="skill">
                <img src="/src/components/icons/arg.svg" alt="Español">
                <span>Español: Nativo</span>
            </li>
            <li class="skill">
                <img src="/src/components/icons/eeuu.svg" alt="Inglés">
                <span>Inglés: Intermedio</span>
            </li>
        </ul>
     </div>
   </div>
</template>
-->
<style scoped>
.skills-contenedor {
    padding: 2rem;
    background: linear-gradient(-45deg, #021526, #03346E, #6EACDA, #E2E2B6);
}

.skills-categoria {
    margin-bottom: 20px;
}

.skills-categoria h3 {
    display: flex;
    justify-content: left;
    margin-bottom: 10px;
    font-size: 1.5em;
    color: aliceblue;
    font-size: 1.5em;
    font-weight: bold;
}

.skills {
    display: flex;
    justify-content: space-evenly;
    flex-wrap: wrap;
    gap: 20px;
}

.skill {
    display: flex;
    justify-content: center;
    align-items: center;
    gap: 10px;
    background-color: rgba(241, 245, 243, 0.856);
    padding: 0.3em;
    border-radius: 8px;
    box-shadow: 0px 4px 12px rgba(2, 151, 151, 0.963);
    flex: 1 1 200px;
    max-width: 250px;
}

.skill:hover {
    background-color: rgba(2, 151, 151, 0.963);
}

.skill img {
    width: 35px;
    height: 35px;
}

.skill span {
    font-size: 1em;
    color: #333;
}

.skill:hover {
    font-size: 1.3em;
    transition: 2s ease;
}

@media (max-width: 768px) {
    .skills-categoria h3 {
        justify-content: center;
    }

    .skill {
        justify-content: center;
    }
}
</style>